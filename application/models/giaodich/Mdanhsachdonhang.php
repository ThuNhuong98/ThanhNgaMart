...
class Mdanhsachdonhang extends CI_Model
{
	public function get_danhsachdonhang(){
		$this->db->select('donhang.*, count(FK_sMaSP) as sosp, sum(iSoLuong*fDonGia) as tongtien, thongtingiaohang.*, taikhoan.sTenTaiKhoan, trangthai.sTenTrangThai');
	...
	public function chuyenTrangThai($madh, $matrangthai = null)
	{
		if (!empty($matrangthai)){
			$this->db->where('PK_sMaDonHang', $madh);
			$this->db->update('donhang', array('FK_iMaTrangThai' => $matrangthai));
			...
			$this->db->where('PK_sMaDonHang', $madh);
			$this->db->update('donhang', array('FK_iMaTrangThai' => $ttTiep));
			return $this->db->affected_rows();
		}
	}
